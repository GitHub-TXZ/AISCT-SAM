...
# add by xz
class DynamicAvgPool3d(nn.Module):
    def __init__(self):
        super(DynamicAvgPool3d, self).__init__()
        # 初始化一个 AvgPool3d 层，但不设置窗口大小和步长
        self.avgpool = nn.AvgPool3d(kernel_size=1, stride=1)

    def forward(self, x):
        # 根据输入 x 的深度维度设置窗口大小
        kernel_size = (x.size(2), x.size(3), x.size(4))
        self.avgpool.kernel_size = kernel_size
...
class PlaneFinder(nn.Module):
    ...
    def forward(self, x):
        ...
        new_view = torch.cat([view[:, :1] * math.pi / 180 * self.x_rotation_range,
                              ...
                              view[:, 2:3] * math.pi / 180 * self.z_rotation_range,  # 最大旋转角度（-40到+40）
                              view[:, 3:4] * self.x_translation_range,  # 最大平移距离（-0.5到+0.5，也就是半张图）
...
if __name__ == "__main__":
    ...
    model.cuda()
    dummy_x = torch.rand((4, 1, 30, 256, 256)).cuda()
    flops, params = profile(model, inputs=(dummy_x,))
    print(f"Total Parameters(M): {params / 1e6}")
    print(f"Total GFLOPs: {flops / 1e9}")
    summary(model, input_size = (1, 40, 256, 256))
    out = model(dummy_x)
    print(out)