...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        hidden_output = []
        hidden_output.append(x) # 原始输入加上12个transformer block的输出
        ...
        return x, hidden_output # 返回经过neck的输出和原始输入加上transformer block的输出
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.adapter_conv = nn.Conv3d(self.adapter_channels, self.adapter_channels, kernel_size=(3, 1, 1),
                                      padding='same')
        ...
        self.adapter_conv_2 = nn.Conv3d(self.adapter_channels, self.adapter_channels, kernel_size=(3, 1, 1),
                                        padding='same')
    ...
    def forward(self, x: torch.Tensor, d_size) -> torch.Tensor:
        ...
        x = self.adapter_linear_down(x) #[b*d, h, w, c]
        x = x.contiguous().view(int(b_size/d_size), d_size, hw_size, hw_size, self.adapter_channels) # [b, d, h, w, c]
        x = torch.permute(x, (0, -1, 1, 2, 3)) # [b, c, d, h, w]
        x = self.adapter_conv(x)
        x = torch.permute(x, (0, 2, 3, 4, 1)) # [b, d, h, w, c]
        x = x.contiguous().view(b_size, hw_size, hw_size, self.adapter_channels) # [b*d, h, w, c]
