...
def window_reverse(windows, window_size, S, H, W):
    ...
    return x
# (960, 4, 4, 4, 192) --> (1,3,20,16,4,4,4,192) 然后再将4,4,4的维度转换回去
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        x = F.pad(x, (0, 0, 0, pad_r, 0, pad_b, 0, pad_g)) # 注意是从后往前pad
        _, Sp, Hp, Wp, _ = x.shape

        # cyclic shift
        if self.shift_size > 0:     #先对patch进行循环移位，然后再划分窗口
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, S, H, W):
        ...
        s_slices = (slice(0, -self.window_size),
                    ...
                    slice(-self.shift_size, None))          # (start, end, increment)
...
class nnFormer(nn.Module):
    ...
    def __init__(self, crop_size=[32, 256, 224],
                 embedding_dim=192,
                 input_channels=1,
                 num_classes=2,
                 conv_op=nn.Conv3d,
...
if __name__ == '__main__':
    # 需要修改crop_size=[64, 320, 256]和num class=2
    import os
    ...
    model = nnFormer().to(device)
    input_shape = (1, 1, 32, 256, 224)  # (batch_size, channels, height, width, depth)
