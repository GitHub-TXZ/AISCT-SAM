...
class SkipDenseNet3D(nn.Module):
    ...
    def __init__(self, in_channels=1, classes=3, growth_rate=16, block_config=(4, 4, 4, 4), num_init_features=32, drop_rate=0.1,
                 bn_size=4):
...
if __name__ == '__main__':
    # 输入会经过 2、4、8、16倍下采样，因此输入必须是16的倍数
    import torch
    from torchsummary import summary
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = SkipDenseNet3D(in_channels=1, classes=3, growth_rate=16, block_config=(4, 4, 4, 4), num_init_features=32, drop_rate=0.1,
                 bn_size=4).to(device)
    #生成伪3D数据测试模型
