...
class WindowAttention3D(nn.Module):
    ...
    def forward(self, x, mask=None, prev_v=None, prev_k=None, prev_q=None, is_decoder=False):
        ...
        if mask is not None:
            ...
            attn = self.softmax(attn)   # dim = -1 时，是按照行进行 概率分布计算



        attn = self.attn_drop(attn)
        # 这里先转置再reshape
        # 因为张量（250,3,343,32）reshape到（250，343，96）和（250,343,3,32）进行reshape到（250，343，96）效果是不相同的
...
class PatchExpand_Up(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(B, 64 // self.D_ratio, H, W, C)
