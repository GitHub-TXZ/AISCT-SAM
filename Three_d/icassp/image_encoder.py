...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        b, c, d, h, w = x.shape  # c=2
        
        x_sym = x[:, 1, :, :, :].unsqueeze(1) 
        x_sym = x_sym.repeat(1, 3, 1, 1, 1).permute(0, 2, 1, 3, 4) .contiguous().view(b*d, 3, h, w)
        
        x = x[:, 0, :, :, :].unsqueeze(1)  # 第一个通道是原图
        x = x.repeat(1, 3, 1, 1, 1).permute(0, 2, 1, 3, 4) .contiguous().view(b*d, 3, h, w)
        
        x = self.patch_embed(x)
        x_sym = self.patch_embed(x_sym)
        diff_token = x - x_sym
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, d_size) -> torch.Tensor:
        shortcut = x
        depth_x = self.depth_branch(x, d_size)
        x = self.norm1(x)
        # Window partition
