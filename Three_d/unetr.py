...
class UNETR(nn.Module):
    def __init__(self, img_shape=(32, 256, 224), input_dim=1, output_dim=2, embed_dim=768, patch_size=16, num_heads=12, dropout=0.1):
        super().__init__()
...
if __name__ == "__main__":
    #UNETR 下采样倍数：1/16. 1/16. 1/16  参数量155M, 运行summary时需要去掉attn里返回的, weights
    import os
    os.environ['CUDA_VISIBLE_DEVICES'] = '0'
    from torchsummary import summary

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    x = torch.Tensor(1, 1, 32, 256, 224)
    x = x.to(device)
    ...
    # summary(model, input_size=(4, 128, 128, 128), device=device.type)