...
from .hybridformer import Coformer3D
...
class Generic_UNet(nn.Module):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        scale_factor = [(0.5, 0.5, 0.5), (0.5, 0.5, 0.5), (0.5, 0.5, 0.5), (0.5, 0.5, 0.5)]
        self.trans_encoder = Coformer3D(patch_size=(2, 2, 2), scale_factor=scale_factor, in_chans=input_channels,
        ...
        self.seg_outputs = []
        self.fuse = []
        self.mfa1 = nn.Conv3d(768, 320, 1)  # Last 3 levels
    ...
    def forward(self, x):
        ...
        # feat_trans = [F.interpolate(i, size=(5, 7, 7), mode='trilinear') for i in x_trnas[-2:-1]]
        feat_trans = [F.interpolate(i, size=(8, 8, 8), mode='trilinear') for i in x_trnas[-2:-1]]
        feat_trans = self.mfa2(torch.cat(feat_trans + [x_trnas[-1]], dim=1))
        # feat_conv = [F.interpolate(i, size=(5, 7, 7), mode='trilinear') for i in skips[-3:]]
        feat_conv = [F.interpolate(i, size=(4, 4, 4), mode='trilinear') for i in skips[-3:]]
