...
class Generic_UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        feat_trans = [F.interpolate(i, size=(10, 6, 5), mode='trilinear') for i in x_trnas[-2:-1]]
        feat_trans = self.mfa2(torch.cat(feat_trans + [x_trnas[-1]], dim=1))
        feat_conv = [F.interpolate(i, size=(5, 6, 5), mode='trilinear') for i in skips[-3:]]
        feat_conv = self.mfa1(torch.cat(feat_conv + [x], dim=1))
        feat_trans = nn.functional.interpolate(feat_trans, scale_factor=(0.5, 1, 1), mode='trilinear')
