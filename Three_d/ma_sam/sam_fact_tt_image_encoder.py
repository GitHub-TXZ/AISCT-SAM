...
class Fact_tt_Sam(nn.Module):
    ...
    def __init__(self, sam_model: Sam = sam, r: int = 64, fact_layer=None, s=1):  # s是尺度系数
    ...
    def forward(self, batched_input, multimask_output=False, image_size=512):
        if batched_input.shape[1] == 1:
            batched_input = batched_input.repeat(1, 3, 1, 1, 1)
        b = batched_input.shape[0]  # batch size
        batched_input = batched_input.permute(0, 2, 1, 3, 4)
        temp_return = self.sam(batched_input, multimask_output, image_size)
        return temp_return['masks'].view(b, 2, -1, image_size, image_size)
