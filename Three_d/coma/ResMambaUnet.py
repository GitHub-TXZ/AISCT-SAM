...
import numpy as np
from Three_d.coma import CNNBackbone
from .position_encoding import build_position_encoding
from Three_d.coma.Mamba.mamba import ResMambaBlock
...
class CoMa(nn.Module):
    def __init__(self, norm_cfg='BN', activation_cfg='ReLU', img_size=None, num_classes=None, weight_std=False, deformable=True):
        ...
        self.backbone = CNNBackbone.Backbone(depth=9, norm_cfg=norm_cfg, activation_cfg=activation_cfg, weight_std=weight_std,deformable=deformable)
        ...
        self.encoder_Mamba =ResMambaBlock(in_channels=384)
        total = sum([param.nelement() for param in self.encoder_Mamba.parameters()])
    ...
    def forward(self, inputs):
        ...
        x_fea_pos = [i+j for i,j in zip(x_fea, x_posemb)]
        x_fea_pos_fla = [i.view(i.shape[0], i.shape[1],-1).transpose(-1,-2) for i in x_fea_pos]
        x_trans = self.encoder_Mamba(torch.cat(x_fea_pos_fla, dim=1))
...
class ResMambaUnet(nn.Module):
    """
    ResMambaUnet
    """#LeakyReLU
    def __init__(self, norm_cfg='IN', activation_cfg='ReLU', img_size=None, num_classes=2, weight_std=False, deep_supervision=False):
        super().__init__()
        self.do_ds = False
        self.CoMa = CoMa(norm_cfg, activation_cfg, img_size, num_classes, weight_std) # CoMa
