...
class Attention(nn.Module):
    def __init__(self,
        ...
        self.conv_q = nn.Conv3d(channels, channels, kernel_size, stride_q, padding_q, bias=attention_bias,
                                groups=channels)
        self.layernorm_q = nn.LayerNorm(channels, eps=1e-5)
        self.conv_k = nn.Conv3d(channels, channels, kernel_size, stride_kv, stride_kv, bias=attention_bias,
                                groups=channels)
        self.layernorm_k = nn.LayerNorm(channels, eps=1e-5)
        self.conv_v = nn.Conv3d(channels, channels, kernel_size, stride_kv, stride_kv, bias=attention_bias,
                                groups=channels)
    ...
    def forward(self, x):
        ...
        # x1 = torch.nn.functional.scaled_dot_product_attention(query=q, value=v, key=k, is_causal=True, dropout_p=0.0)
        x1 = x1[0].permute(0, 2, 1)
        x1 = x1.view(x1.shape[0], x1.shape[1], np.cbrt(x1.shape[2]).astype(int), np.cbrt(x1.shape[2]).astype(int),
                     np.cbrt(x1.shape[2]).astype(int))
...
class Wide_Focus(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv2 = nn.Conv3d(in_channels, out_channels, 3, 1, padding="same", dilation=2)
        self.conv3 = nn.Conv3d(in_channels, out_channels, 3, 1, padding="same", dilation=3)
...
class FCT3D(nn.Module):
    def __init__(self):
        ...
        # init_sizes = init_sizes.permute(0, 3, 1, 2) #(2,1,224,224)
