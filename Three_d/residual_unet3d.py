...
class Residual_UNet3D(nn.Module):
    """
    Implementations based on the Unet3D paper: https://arxiv.org/pdf/1706.00120.pdf
    """

    def __init__(self, in_channels=1, n_classes=3, base_n_filter=8):
        super(Residual_UNet3D, self).__init__()
...
if __name__ == '__main__':
    ...
    model = Residual_UNet3D(in_channels=1, n_classes=3).to(device)
    # 生成伪3D数据测试模型
    ...
    summary(model, input_size=(1, 1, 128, 128, 128), device=device.type)
    # print(model)
