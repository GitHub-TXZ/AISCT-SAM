...
def patch_embed(inc, outc):
    x = StdConv2d(inc, outc, kernel_size=1, stride=1)
    return x
...
class UnetEncoder(nn.Module):
    def __init__(self, n_channels=1, width=64):
...
class ResNetV2(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(len(self.body) - 1):  # range(2)
            x = self.body[i](x)
            right_size = int(in_size1 / 4 / (i + 1))
            right_size2 = int(in_size2 / 4 / (i + 1))
            if x.size()[2] != right_size:
                pad = right_size - x.size()[2]
                assert 3 > pad > 0, "x {} should {}".format(x.size(), right_size)
                feat = torch.zeros((b, x.size()[1], right_size, right_size2), device=x.device)
...
class Attention(nn.Module):
    def __init__(self, vis, hidden_size):
...
class Embeddings(nn.Module):
    ...
    def __init__(self, img_size):
        ...
        self.hybrid = True
        self.hybrid_model = hyencoder()
        self.patch_embeddings_768 = patch_embed(1024, 768)
        self.patch_embeddings_384 = patch_embed(512, 384)
        self.patch_embeddings_192 = patch_embed(256, 192)
        self.patch_embeddings_96 = patch_embed(128, 96)
        self.dropout = Dropout(dropout_rate)

    def forward(self, x):

        features = self.hybrid_model(x)
        hidden_size = [768, 384, 192, 96]
        factors = [16, 8, 4, 2]
        n_patches = (192//16 * 192//16, 192//8 * 192//8, 192//4 * 192//4, 192//2 * 192//2,)
        features[0] = self.patch_embeddings_768(features[0])
        features[1] = self.patch_embeddings_384(features[1])
        features[2] = self.patch_embeddings_192(features[2])
        # features[3] = self.patch_embeddings_96(features[3])
        for i in range(t):

            features[i] = features[i].flatten(2).transpose(-1, -2)
            # x5 = x5.transpose(-1, -2)  # (B, n_patches, hidden)
            position_embeddings = nn.Parameter(torch.zeros(1, n_patches[i], hidden_size[i])).cuda()
            features[i] = features[i] + position_embeddings
            if i == 0:
                features[i] = self.dropout(features[i])
...
class Encoder(nn.Module):
    def __init__(self, vis, hidden_size, num_layers):
        super(Encoder, self).__init__()
        self.vis = vis
        self.layer = nn.ModuleList()
        self.encoder_norm = LayerNorm(hidden_size, eps=1e-6)
        for _ in range(num_layers):
            layer = Block(vis, hidden_size=hidden_size)
...
class Transformer(nn.Module):
    def __init__(self, img_size, vis, hidden_size):
        ...
        for i in range(t):
            self.blocks.append(
                Encoder(vis, hidden_size=hidden_size[i], num_layers=num_layers[i])
            )

    def forward(self, input_ids):
        features = self.embeddings(input_ids)
        for i, blk in enumerate(self.blocks):
            features[i] = blk(features[i])  # (B, n_patch, hidden)
        return features
...
class DecoderCup(nn.Module):
    def __init__(self):
        super().__init__()
        head_channels = 512
        skip_channels = [256, 128, 128, 0]
        self.block = nn.ModuleList()

        self.conv_more_768 = Conv2dReLU(768, 512, kernel_size=3, padding=1, use_batchnorm=True,)
        self.conv_more_384 = Conv2dReLU(384, 256, kernel_size=3, padding=1, use_batchnorm=True,)
        self.conv_more_192 = Conv2dReLU(192, 128, kernel_size=3, padding=1, use_batchnorm=True,)
        self.conv_more_96 = Conv2dReLU(96, 64, kernel_size=3, padding=1, use_batchnorm=True,)
        in_channels = [512, 512, 384, 256]  # [512, 256, 128, 64]
        out_channels = [512, 384, 256, 32]  # [256, 128, 64, 16]

        if n_skip != 0:
            for i in range(4 - n_skip):  # re-select the skip channels according to n_skip
                skip_channels[3 - i] = 0  # skip_channels=[xx00]
    ...
    def forward(self, features):
        H = (12, 24, 48, 96)
        W = (12, 24, 48, 96)
        n_patch = (12*12, 24*24, 48*48)
        for i in range(t):
            B, _, hidden = features[i].size()  # reshape from (B, n_patch, hidden) to (B, h, w, hidden)
            h, w = H[i], W[i]
            features[i] = features[i].permute(0, 2, 1)
            features[i] = features[i].contiguous().view(B, hidden, h, w)
        features[0] = self.conv_more_768(features[0])
        features[1] = self.conv_more_384(features[1])
        features[2] = self.conv_more_192(features[2])
        # features[3] = self.conv_more_96(features[3])
        x = features[0]
...
class FRPNet(nn.Module):
    def __init__(self, img_size=(192, 192), num_classes=1, n_channels=1, zero_head=False, vis=False, ):
        super(FRPNet, self).__init__()
        self.n_channels = n_channels
        self.n_classes = num_classes
        self.zero_head = zero_head
        self.classifier = classifier
        self.transformer = Transformer(img_size, vis, hidden_size=[768, 384, 192, 96])
    ...
    def load_from(self, weights,):
        with torch.no_grad():
            ...
            posemb_new = self.transformer.embeddings.position_embeddings  # 1 196 768
