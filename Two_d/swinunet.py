...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        # 这里相当于把原来的4个token的信息通过一个MLP进行融合
        x = self.norm(x)
...
class PatchExpand(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(B, H, W, C)
        # 为什么要先rearrage后View, Maybe考虑到View只是改变数据显示方式，而没有考虑到原始数据的排序方式。
...
class SwinUnet(nn.Module):
    ...
    def __init__(self, config = config, img_size=224, num_classes=2, zero_head=False, vis=False):
...
if __name__ == '__main__':
    ...
    input_shape = (1, 3, 224, 224)  # (batch_size, channels, height, width, depth)
