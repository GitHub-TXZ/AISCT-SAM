...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, skip=None):
        x = self.up(x)    # 这里是直接应用双线性插值，而不是使用反转卷积或者patch expanding将C维度的特征压缩到空间维度
...
class DecoderCup(nn.Module):
    ...
    def forward(self, hidden_states, features=None):
        ...
        x = x.contiguous().view(B, hidden, h, w)     # 直接把Trans的特征维度放前面来，将npatchs展开为图像大小，然后当成卷积特征直接上采样
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.size()[1] == 1:
            x = x.repeat(1, 3, 1, 1)
        # 输入到Transformer的特征图大小是16*16
        x, attn_weights, features = self.transformer(x)  # (B, n_patch, hidden)

        x = self.decoder(x, features)  #最后一个bolock的输出是16通道

        logits = self.segmentation_head(x)  #head才输出到class通道
...
def get_r50_b16_config():
    ...
    config.pretrained_path = "/home/tanxz/Model_Checkpoint/R50+ViT-B_16.npz"
...
class TransUNet(VisionTransformer):
    def __init__(self):
        ...
        super(TransUNet, self).__init__(config=config,img_size=224, num_classes=21843, zero_head=False, vis=False)
        super().load_from(weights=np.load(config.pretrained_path))
...
if __name__ == '__main__':
    ...
    # transunet我写死了只支持224*224
    # 还有就是修改类别数只需查找get_r50_b16_config()里面的n_class
    input_shape = (1, 1, 224, 224)  # (batch_size, channels, height, width, depth)
    # input_shape = (1, 1, 256, 224)
